...
import { fetchRepuestoBySlug } from "../../../services/repuestosService";

const mapRepuestoData = (data) => {
  if (!data) return null;
  return {
    slug: data.slug,
    ...
    heroDescription: data.hero_description ?? data.heroDescription ?? data.description ?? "",
    priceSoles: data.price_soles ?? data.priceSoles ?? "",
    priceUsd: data.price_usd ?? data.priceUsd ?? "",
    price: data.price ?? data.precio ?? null,
    specPrimary: data.spec_primary ?? data.specPrimary ?? "",
    specSecondary: data.spec_secondary ?? data.specSecondary ?? "",
    specTertiary: data.spec_tertiary ?? data.specTertiary ?? "",
...
const RepuestoDetail = () => {
  ...
  useEffect(() => {
    const loadRepuesto = async () => {
      setIsLoading(true);
      const { data, error } = await fetchRepuestoBySlug(slug);
      const dataRepuesto = !error && data ? mapRepuestoData(data) : null;
      const stateRepuesto = mapRepuestoData(repuestoFromState);
      const combined = dataRepuesto || stateRepuesto || null;
      setRepuesto(combined);
      setIsLoading(false);
    };

    loadRepuesto();
  }, [slug, repuestoFromState]);

  useEffect(() => {
    if (repuesto?.title) {
      document.title = `Land Roys | ${repuesto.title}`;
      const description = repuesto.heroDescription || repuesto.description;
      setMetaDescription(description);
      setMetaProperty("og:title", repuesto.title);
      setMetaProperty("og:description", description);
      setMetaProperty("og:image", repuesto.heroImage || repuesto.image);
      setMetaProperty("og:type", "product");
      setMetaProperty("og:url", window.location.href);
    }
  }, [repuesto]);

  if (!repuesto && !isLoading) {
    return (
  ...
  const specs = useMemo(() => (repuesto ? [
    { label: "Beneficio clave", value: repuesto.specPrimary },
    { label: "Atributo técnico", value: repuesto.specSecondary },
    { label: "Durabilidad", value: repuesto.specTertiary },
  ].filter((item) => item.value) : []), [repuesto]);

  const highlights = repuesto?.marketingHighlights?.filter(Boolean) ?? [];
  const galleryImages = repuesto?.galleryImages?.filter(Boolean) ?? [];
  const videoUrls = repuesto?.videoUrls?.filter(Boolean) ?? [];
  const heroTitle = repuesto?.heroTitle || repuesto?.title || "";
  const heroHighlight = repuesto?.heroHighlight || "";
  const heroImage = repuesto?.heroImage || repuesto?.image || "";
  const primaryImage = repuesto?.image || repuesto?.heroImage || "";
  const priceSoles = repuesto?.priceSoles || (repuesto?.price ? `S/ ${repuesto.price.toLocaleString("es-PE")}` : "");
  const priceUsd = repuesto?.priceUsd || "";
      ...
      <section className="py-24 px-6 lg:px-20 bg-background-light dark:bg-background-dark">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="flex flex-col gap-8 order-2 lg:order-1">
            <div>
              {repuesto.heroTagline && (
                <span className="text-primary font-bold tracking-widest uppercase text-sm">{repuesto.heroTagline}</span>
              )}
            ...
            {highlights.map((highlight, index) => (
              <div className="flex-1 min-w-[300px] group cursor-default" key={`${highlight}-${index}`}>
                <div className="relative flex flex-col gap-4 p-8 rounded-xl bg-dark-gray border border-white/5 hover:border-primary/40 transition-all">
                  ...
                  <p className="text-gray-400 text-sm">Detalle destacado del repuesto.</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="max-w-6xl mx-auto flex flex-col gap-12">
            {galleryImages.length > 0 && (
              <div>
                ...
                <div className="repuesto-gallery-grid">
                  {galleryImages.map((imageUrl, index) => (
                    <div key={`${imageUrl}-${index}`} className="repuesto-gallery-item">
                      <img src={imageUrl} alt={`${repuesto.title} ${index + 1}`} loading="lazy" />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {videoUrls.length > 0 && (
              <div>
                ...
                <div className="repuesto-video-grid">
                  {videoUrls.map((url, index) => {
                    const embedUrl = getEmbedUrl(url);
                    return (
                      <div key={`${url}-${index}`} className="repuesto-video-card">
                        {embedUrl ? (
                          <iframe
                            src={embedUrl}
                            title={`${repuesto.title} video ${index + 1}`}
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                            allowFullScreen
                          ></iframe>
                        ) : (
                          <a href={url} target="_blank" rel="noreferrer" className="repuesto-video-link">
                            Ver video {index + 1}
                          </a>
                        )}
                      </div>
